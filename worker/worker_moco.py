...
from .train.pretrain_moco import *
...
def pretrain_worker_rcrop(rank, world_size, cfg):
    ...
    if local_args.local_rank == 0:
        ...
        #     dm.save_test_ckp(model, optimizer, 'moco_state')
        # end ####################
    for epoch in range(local_args.start_epoch, local_args.run_p.pretrain_epoch + 1):
        ...
        if local_args.local_rank == 0:
            if not local_args.ifTry:
                # test ####################
                # if epoch % int(0.2*local_args.run_p.pretrain_epoch) == 0:
                #     dm.save_test_ckp(model, optimizer, 'moco_state')
                # end ####################
                dm.save_ckp(model, optimizer, 'moco_state')
...
def pretrain_worker_rcrop_amp(rank, world_size, cfg):
    ...
    for epoch in range(local_args.start_epoch, local_args.run_p.pretrain_epoch + 1):
        ...
        loss, batch_time = pretrain_amp_f16(model, pretrain_loader, optimizer, criterion, local_args, epoch, scaler)
...
def pretrain_worker_ccrop(rank, world_size, cfg):
    ...
    for epoch in range(local_args.start_epoch, local_args.run_p.pretrain_epoch + 1):
        ...
        if local_args.local_rank == 0:
            if not local_args.ifTry:
                dm.save_ckp(model, optimizer, 'moco_state', ['boxes', pretrain_set.boxes])
            dm.end_pretrain_epoch(model.module, loss, batch_time)


def pretrain_worker_ccrop_amp(rank, world_size, cfg):
    print('==> Start rank:', rank)
    local_args = copy.copy(cfg)
    local_args.local_rank = rank % world_size
...
def pretrain_worker_Fcrop(rank, world_size, cfg):
    ...
    for epoch in range(local_args.start_epoch, local_args.run_p.pretrain_epoch + 1):
        ...
        if local_args.local_rank == 0:
            if not local_args.ifTry:
                dm.save_ckp(model, optimizer, 'moco_state', ['centers', pretrain_set.centers])
            dm.end_pretrain_epoch(model.module, loss, batch_time)
...
def pretrain_worker_Fcrop_sstd(rank, world_size, cfg):
    ...
    if local_args.local_rank == 0:
        ...
        ####################################
        # if not local_args.ifTry:
        #     dm.save_test_ckp(model, optimizer, 'moco_state', ['centers', pretrain_set.centers])
        ####################################
...
def pretrain_worker_Fcrop_amp(rank, world_size, cfg):
    print('==> Start rank:', rank)
    local_args = copy.copy(cfg)
    local_args.local_rank = rank % world_size
...
def pretrain_worker_Fcrop_amp_sstd(rank, world_size, cfg):
    print('==> Start rank:', rank)
    local_args = copy.copy(cfg)
    local_args.local_rank = rank % world_size
